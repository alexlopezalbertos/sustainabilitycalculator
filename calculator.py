...
c1, c2 = st.columns(2)
with c1:
    su_factor_c = st.number_input("Current Case SU factor", step=0.001, format="%.3f")
    items_case_c = st.number_input("Current Items/Case", step=1, format="%d")
with c2:
    su_factor_n = st.number_input("New Case SU factor", step=0.001, format="%.3f")
    items_case_n = st.number_input("New Items/Case", step=1, format="%d")
...
if "🚢 Sea" in transport_modes:
    c1, c2 = st.columns(2)
    with c1:
        cases_sea_c = st.number_input("Current number of Cases/Container", step=1, format="%d")
    with c2:
        cases_sea_n = st.number_input("New number of Cases/Container", step=1, format="%d")
...
if "🚚 Road" in transport_modes:
    ...
    with c1:
        pallet_type_eu_c = st.selectbox("Current EU pallet type", ["B1", "B2", "N/A"], index=None)
    with c2:
        if pallet_type_eu_c == "B1":
            cases_b1_c = st.number_input("Current Cases/Pallet B1", step=1, format="%d")
            cases_per_truck_current_eu = cases_b1_c * pallet_factors["B1"]
        elif pallet_type_eu_c == "B2":
            cases_b2_c = st.number_input("Current Cases/Pallet B2", step=1, format="%d")
            cases_per_truck_current_eu = cases_b2_c * pallet_factors["B2"]

    with c3:
        pallet_type_eu_n = st.selectbox("New EU pallet type", ["B1", "B2", "N/A"], index=None)
    with c4:
        if pallet_type_eu_n == "B1":
            cases_b1_n = st.number_input("New Cases/Pallet B1", step=1, format="%d")
            cases_per_truck_new_eu = cases_b1_n * pallet_factors["B1"]
        elif pallet_type_eu_n == "B2":
            cases_b2_n = st.number_input("New Cases/Pallet B2", step=1, format="%d")
            cases_per_truck_new_eu = cases_b2_n * pallet_factors["B2"]

    # UK Pallets
    c5, c6, c7, c8 = st.columns(4)
    with c5:
        pallet_type_uk_c = st.selectbox("Current UK pallet type", ["C1", "C2", "N/A"], index=None)
    with c6:
        if pallet_type_uk_c == "C1":
            cases_c1_c = st.number_input("Current Cases/Pallet C1", step=1, format="%d")
            cases_per_truck_current_uk = cases_c1_c * pallet_factors["C1"]
        elif pallet_type_uk_c == "C2":
            cases_c2_c = st.number_input("Current Cases/Pallet C2", step=1, format="%d")
            cases_per_truck_current_uk = cases_c2_c * pallet_factors["C2"]

    with c7:
        pallet_type_uk_n = st.selectbox("New UK pallet type", ["C1", "C2", "N/A"], index=None)
    with c8:
        if pallet_type_uk_n == "C1":
            cases_c1_n = st.number_input("New Cases/Pallet C1", step=1, format="%d")
            cases_per_truck_new_uk = cases_c1_n * pallet_factors["C1"]
        elif pallet_type_uk_n == "C2":
            cases_c2_n = st.number_input("New Cases/Pallet C2", step=1, format="%d")
...
c1, c2 = st.columns(2)
with c1:
    case_weight_c = st.number_input("Current Case Weight [kg]")
with c2:
    case_weight_n = st.number_input("New Case Weight [kg]")
...
try:
    ...
    with c1:
        if "Corrugate (Case)" in material_types:
            kg_corrugate_c = st.number_input("Current corrugate kg/case", format="%.3f")
        if "Carton" in material_types:
            kg_carton_c = st.number_input("Current carton kg/case", format="%.3f")
        if "Paper" in material_types:
            kg_paper_c = st.number_input("Current paper kg/case", format="%.3f")
        if "Plastic" in material_types:
            kg_plastic_c = st.number_input("Current plastic kg/case", format="%.3f")
        if "Glass" in material_types:
            kg_glass_c = st.number_input("Current glass kg/case", format="%.3f")
        if "Metal" in material_types:
            kg_metal_c = st.number_input("Current metal kg/case", format="%.3f")
        if "Other" in material_types:
            kg_other_c = st.number_input("Current other kg/case", format="%.3f")
    with c2:
        if "Corrugate (Case)" in material_types:
            kg_corrugate_n = st.number_input("New corrugate kg/case", format="%.3f")
            material_data["Corrugate (Case)"] = (cases_total_c * kg_corrugate_c,
                                                cases_total_n * kg_corrugate_n)
        if "Carton" in material_types:
            kg_carton_n = st.number_input("New carton kg/case", format="%.3f")
            material_data["Carton"] = (items_total_c * kg_carton_c, items_total_n * kg_carton_n)
        if "Paper" in material_types:
            kg_paper_n = st.number_input("New paper kg/case", format="%.3f")
            material_data["Paper"] = (items_total_c * kg_paper_c, items_total_n * kg_paper_n)
        if "Plastic" in material_types:
            kg_plastic_n = st.number_input("New plastic kg/case", format="%.3f")
            material_data["Plastic"] = (items_total_c * kg_plastic_c, items_total_n * kg_plastic_n)
        if "Glass" in material_types:
            kg_glass_n = st.number_input("New glass kg/case", format="%.3f")
            material_data["Glass"] = (items_total_c * kg_glass_c, items_total_n * kg_glass_n)
        if "Metal" in material_types:
            kg_metal_n = st.number_input("New metal kg/case", format="%.3f")
            material_data["Metal"] = (items_total_c * kg_metal_c, items_total_n * kg_metal_n)
        if "Other" in material_types:
            kg_other_n = st.number_input("New other kg/case", format="%.3f")
            material_data["Other"] = (items_total_c * kg_other_c, items_total_n * kg_other_n)
except:
    st.info("Please fill in all inputs above before continuing")

# ======================= 6. MATERIAL SAVINGS TABLE ========================
try:
    # Only show the subheader if there's at least one material selected
    if material_data:
        st.subheader("📊 Material Savings Summary")

        def highlight_savings_rows(row):
            if row.name in ["Saving [kg]", "Saving [%]"]:
                return ["background-color: #d4edda"] * len(row)
            else:
                return [""] * len(row)

        for material, (current_kg, new_kg) in material_data.items():
            saving_kg = current_kg - new_kg
            saving_pct = ((saving_kg / current_kg) * 100) if current_kg else 0

            df = pd.DataFrame({
                "": ["Current [kg]", "New [kg]", "Saving [kg]", "Saving [%]"],
                f"{material}": [
                    round(current_kg, 2),
                    round(new_kg, 2),
                    round(saving_kg, 2),
            ...
            styled_df = df.style.apply(highlight_savings_rows, axis=1).format("{:.2f}")

            st.write(f"**{material} savings:**")
...
try:
    ...
    if su_factor_c and su_factor_c > 0:
        ...
        def trucks_and_weights_current(row):
            ...
            total_weight = trucks * weight_per_truck
            return pd.Series([trucks, weight_per_truck, total_weight])

        df_current[["Trucks", "Weight/Truck [kg]", "Total Shipped Weight [kg]"]] = df_current.apply(trucks_and_weights_current, axis=1)
...
try:
    ...
    if su_factor_n and su_factor_n > 0:
        ...
        df_new[["Trucks", "Weight/Truck [kg]", "Total Shipped Weight [kg]"]] = df_new.apply(trucks_and_weights_new, axis=1)
    ...
    st.info(f"Please fill in all inputs above before continuing")
...
try:
    ...
    if su_factor_c and su_factor_n and su_factor_c > 0 and su_factor_n > 0:
        ...
        def highlight_green(val):
            return "background-color: #d4edda" if isinstance(val, (float, int)) else ""

        styled_df = df_savings.style.applymap(highlight_green, subset=["% Trucks Saved", "% CO2e Saved"]) \
                                    .format({"% Trucks Saved": "{:.1f}%", "% CO2e Saved": "{:.1f}%"})
